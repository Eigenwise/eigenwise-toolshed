...
Deleting a global category does not delete local `OVERRIDE` rows. A retained override becomes a dangling record: preserve it, omit it from the effective taxonomy, and report `{ kind: 'dangling-override', id, project }` in category-management reads. A `DETACH` row keeps its full local snapshot and remains effective for the project; when the global row with the same ID still exists, report `{ kind: 'shadows-global', id }` because the detached snapshot intentionally shadows global policy. Existing tickets with the deleted ID retain that persisted ID and continue through the existing unknown-category-to-`general` projection ([`lib/store.js:493-540`](../lib/store.js#L493-L540)). If a global row with the same ID is later restored, its retained local override applies again. A project-local `ADD` may use an ID only after no global row exists. Relinking removes the project-local `OVERRIDE` or `DETACH` row and restores global inheritance; relinking a detached category discards its local snapshot. `DISABLE` rows remain project-local management records and are not included in the dangling-override warning.
