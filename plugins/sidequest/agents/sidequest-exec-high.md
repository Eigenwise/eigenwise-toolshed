...
1. **Claim first**: `sidequest claim <ref> --by <worker-id> --executor sidequest-exec-high --effort high --project <project>`
   (add `--executor sidequest-exec-high` and `--effort high` even if the command you were handed omits them. The board refuses the claim if this agent is not the ticket's authoritative generated executor or its derived effort is not `high`.)
