...
1. **Claim first**: `sidequest claim <ref> --by <worker-id> --executor sidequest-exec-medium --effort medium --project <project>`
   (add `--executor sidequest-exec-medium` and `--effort medium` even if the command you were handed omits them. The board refuses the claim if this agent is not the ticket's authoritative generated executor or its derived effort is not `medium`.)
...
arrives, note it in your report and keep to your ticket.
