...
description: >-
  Executes one sidequest ticket at xhigh reasoning effort. Spawn it with an explicit model param
  (the ticket's ⚙tier) so model and effort compose — e.g. subagent_type sidequest-exec-xhigh +
  model sonnet runs the ticket on sonnet at xhigh effort. Pass the ticket ref, the sidequest CLI
...
4. **Close**: `sidequest done <ref> --by <same-worker-id> --model <your tier> --effort xhigh --project <project>`
