...
description: >-
  Executes one sidequest ticket at max reasoning effort. Spawn it with an explicit model param
  (the ticket's ⚙tier) so model and effort compose — e.g. subagent_type sidequest-exec-max +
  model sonnet runs the ticket on sonnet at max effort. Pass the ticket ref, the sidequest CLI
...
4. **Close**: `sidequest done <ref> --by <same-worker-id> --model <your tier> --effort max --project <project>`
