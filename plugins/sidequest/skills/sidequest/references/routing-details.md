...
- **Codex routes (`exec.model` null):** spawn the EXACT shared dispatch executor the dispatch returned as
  ...
  `[sidequest-route model=... effort=... ticket=...]` marker, which the codex-gateway shim resolves per
  request. It carries the ticket ref used for gateway usage attribution, so the marker must reach the spawn prompt intact, and one spawn carries exactly one
