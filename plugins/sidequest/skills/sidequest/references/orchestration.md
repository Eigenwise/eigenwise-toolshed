...
- **Read liveness from the board, not notifications.** An idle notification means the executor's queue had a blip, not that work is stuck. The same ping can describe a working, dead, or already-finished executor, so the only reliable signal is board truth. After a fresh spawn, ignore idle pings during a short grace period of about 2–3 minutes, then make one liveness read: use `pulse <ref>` for the ticket's `{claim:{by,at,ageMs}|null, comments, lastComment, git:{commit,dirty}|null}` state. Until `pulse` is available, read claim age, comments, and `git log`. If several tickets need checking, use `changes --since <iso>` for the `{tickets:[...]}` delta, sorted oldest first.
- **Escalate without wake-up nudges.** If the grace period passes with no claim and no commit, stop the executor and respawn it once with the same briefing. Never both message the old executor and spawn a replacement for one ticket. A respawn that still produces no claim is a user-visible failure: surface it, do not try a third spawn, and do not pull substantial work inline by default. `SendMessage` is for new information such as a scope change or unblock, never a "wake up" poke.
- **Use uneven wakeups.** Check a newly spawned executor after about 5–8 minutes so a never-claimed ticket gets caught during the first failure cycle. Once claims exist, use longer steady-state checks. At every wakeup, diff board state with `changes --since <iso>` before deciding what to do next.
- **Clear verified workers.** Every executor idle notification is a cleanup trigger: pulse the ticket, verify its done comment, board state, and git result, then call `TaskStop` in one motion. Executors deliberately kept alive mid-ticket get the same treatment on their own later idle ping. Sweep ALL finished executors, not just the one that pinged, so session exit only stops live work.
