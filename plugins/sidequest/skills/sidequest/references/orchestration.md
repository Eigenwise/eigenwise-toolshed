...
- **Clear verified workers.** Once you have checked the executor's done comment, board state, and git result, call `TaskStop` with its name. This removes the finished worker from the task list so session exit only stops live work.
