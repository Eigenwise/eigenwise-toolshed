...
const ARRAY_FLAGS = /* @__PURE__ */ new Set(["image", "label", "file", "always-in-scope"]);
const ALIASES = {
  t: "title",
  d: "desc",
  p: "priority",
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a.startsWith("--") || a.startsWith("-")) {
      ...
      const BOOL = /* @__PURE__ */ new Set(["json", "brief", "open", "help", "force", "done", "archived", "all", "dry-run", "yolo", "wave", "unclassified", "enabled", "disabled", "no-fallback", "global", "clear", "steal", "shared-tree", "direct", "sweep", "yes", "integration"]);
      if (val === null) {
        if (BOOL.has(key)) {
          opts[key] = true;
          continue;
...
async function cmdCategory(opts, positional) {
  ...
  const details = (categoryId) => ({
    ...
    effective: store.getCategory(categoryId, projectScope ? { project: slug } : void 0),
  ...
  if (action === "list" || action === "ls") {
    ...
    const categories = store.getCategories(listProjectScope ? { project: slug, withState: true } : void 0).map((category) => {
...
function effortDriftReason(slug, idOrRef, claimedEffort) {
  ...
  return {
    ...
    message: `${t.ref} resolves to ${t.model}·${derivedEffort}, but you claimed as ${claimed} effort. Run sidequest dispatch ${t.ref}, then spawn ${execName}${modelHint}. The ticket remains free for that executor.`
  };
}
...
function executorDriftReason(slug, idOrRef, claimedEffort, executorName, token, direct) {
  ...
  return {
    ...
    message: `${t.ref} resolves to ${t.exec.runsLabel} · ${t.effort} (${t.exec.backend}), but ${executorName} is not its generated executor. Run sidequest dispatch ${t.ref}, then spawn ${expected}. The ticket remains free for the authoritative runtime.`
  };
}
...
async function cmdDispatch(opts, positional) {
  ...
  const spawn2 = agentsync.agentSpawn(agent, isolation, resolved && resolved.model, agent, prompt);
...
async function cmdBoardConfig(opts) {
  const { slug, meta } = await resolveProject(opts);
  const result = opts["always-in-scope"] == null ? { ok: true, config: store.boardConfig(slug) } : store.setBoardConfig(slug, { alwaysInScope: opts["always-in-scope"] });
