...
async function cmdGroomClose(opts, positional) {
  ...
  const delivery = opts["delivery-commit"] ? (() => {
    ...
    return missingFragment ? { ok: false, reason: "missing_release_fragment", message: store.missingReleaseFragmentMessage(idOrRef, missingFragment.fragmentPath, missingFragment.plugins) } : { ok: true };
  })() : { ok: true };
  if (!delivery.ok) {
    if (opts.json) {
