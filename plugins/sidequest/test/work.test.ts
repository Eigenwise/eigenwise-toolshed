...
test('executorPrompt renders a legacy manual prefix as an evidence contract', () => {
  const t = ticket({ executorVerifyKind: 'command', executorVerify: 'manual: checked the rendered page' });
  const prompt = work.executorPrompt(t, 'Inspect the ticket.');
  assert.match(prompt, /Verify oracle: manual\nRecord evidence matching: checked the rendered page\./);
  assert.doesNotMatch(prompt, /Verify command:/);
});
