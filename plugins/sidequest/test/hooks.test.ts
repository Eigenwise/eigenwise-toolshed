...
test('session-start: carries evidence-first advisory routing guidance', () => {
  ...
  assert.match(ctx, /Wave mode: BEFORE dispatch, file the COMPLETE planned backlog: all waves’ tickets, declared files, dependency links, per-ticket verify\. Dispatch the ready wave in parallel\. Never drip-file\/dispatch\/wait: serializes work and hides the plan\. Later discoveries file mid-run\./, 'wave mode must file the planned backlog before dispatch');
