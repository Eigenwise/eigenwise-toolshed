...
function main() {
  ...
  if (source === 'compact' || source === 'resume') {
    emit(
      '=== sidequest (active — context restored) ===\n' +
        'Context restored. Reload the Sidequest skill before acting. Substantive work needs a board ticket, then fresh dispatch\'s exact token-gated executor and briefing. Every Agent launch must use that executor. For a tiny lookup, use Read, Glob, Grep, or WebFetch inline; delegated exploration, research, review, or analysis gets a ticket first. Use mcp__plugin_sidequest_board__list with status=doing FIRST; if MCP is unavailable, `' + cli + ' list --status doing`.\n' +
  ...
  emit(
    '=== sidequest (active) ===\n' +
      ...
      '• Route execution DOWN: substantive investigations and changes are board tickets, then fresh `dispatch` returns the exact stable executor, briefing, and token to spawn immediately. Dispatch is instant: no registration/watcher wait. Every Agent launch uses that executor. For a tiny lookup, use Read, Glob, Grep, or WebFetch inline; delegated exploration, research, review, or analysis gets a ticket first. Native Agent results arrive automatically: never TaskOutput them. For liveness use pulse ref / changes --since; TaskStop only after terminal board evidence. Never proxy-wait for an executor: no Bash/PowerShell/Monitor/cron task to wait for it or poll for its report file, and no blocking TaskOutput on that proxy. Agent acknowledgement only means launched: pulse immediately and call it running only after the token claim is visible. Missing claim: diagnose or respawn. Inline only trivial one-step work. Claude passes `model: exec.model`; Codex omits it. Use `bypassPermissions`; do not use `native_agent`.\n' +
