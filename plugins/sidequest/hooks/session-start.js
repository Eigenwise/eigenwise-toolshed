...
function deferralNotice(cwd, progress) {
  ...
  const classification = progress.phase === "classifying" ? ` Classifying ${progress.observed}/${progress.candidates}${progress.current ? `: ${progress.current}` : ""}${progress.reason ? ` (${progress.reason})` : ""}.` : "";
...
// src/lib/sidequest-install.ts
function selectSidequestRegistry(registry, pluginRoot2, claudeHome2) {
  const fs8 = require("node:fs");
  const path8 = require("node:path");
  const officialId = "sidequest@eigenwise-toolshed";
  const canonical = (value) => {
    if (typeof value !== "string" || !value || !path8.isAbsolute(value)) return null;
    let resolved = path8.resolve(value);
    try {
      resolved = fs8.realpathSync(resolved);
    } catch (_) {
    }
    return process.platform === "win32" ? resolved.toLowerCase() : resolved;
  };
  const root = canonical(pluginRoot2);
  const cache = canonical(path8.join(claudeHome2, "plugins", "cache"));
  ...
  const relative = root && cache ? path8.relative(cache, root) : "";
  const parts = relative.split(path8.sep);
  const cacheIdentity = parts.length === 3 && parts[0] !== ".." && parts[1] === "sidequest" && !path8.isAbsolute(relative);
  let pluginId = officialId;
  let sourceCacheFamily = null;
  if (cacheIdentity) {
    pluginId = `sidequest@${parts[0]}`;
  } else if (root) {
    if (!fs8.existsSync(root)) return { pluginId: null, installs: [] };
    const readJson = (file) => {
      try {
    ...
    const known = readJson(path8.join(claudeHome2, "plugins", "known_marketplaces.json"));
    const within = (directory, value) => value === directory || value.startsWith(directory + path8.sep);
    const owners = [];
    for (const [name, metadata] of Object.entries(known && typeof known === "object" ? known : {})) {
      ...
      const catalog = readJson(path8.join(directory, ".claude-plugin", "marketplace.json"));
      ...
      const matching = declarations.filter((entry) => canonical(path8.resolve(directory, entry.source)) === root);
      ...
      owners.push({ pluginId: id, valid, family: canonical(path8.join(claudeHome2, "plugins", "cache", name, "sidequest")) });
    }
  ...
  return {
    ...
    installs: installs.filter((install) => {
      ...
      if (sourceCacheFamily) return candidate === root || path8.dirname(candidate) === sourceCacheFamily;
      return cacheIdentity ? path8.dirname(candidate) === path8.dirname(root) : candidate === root;
    })
  };
}
...
function registryInstalls(projectPath, options = {}) {
  ...
  const { pluginId, installs } = selectSidequestRegistry(registry, options.pluginRoot || process.env.CLAUDE_PLUGIN_ROOT || import_node_path6.default.resolve(__dirname, ".."), claudeHome(options));
