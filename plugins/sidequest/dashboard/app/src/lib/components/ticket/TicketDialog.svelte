...
<style>
	.dialog-content { padding:1rem; } .dialog-header { display:flex; justify-content:space-between; gap:1rem; align-items:start; padding-bottom:1rem; border-bottom:1px solid var(--border); } h2 { margin:0; font-family:var(--font-serif); } small { color:var(--text-muted); } .main-grid { display:grid; grid-template-columns:minmax(0, 1fr) minmax(18rem, .7fr); gap:1.25rem; margin-top:1rem; } .fields { display:grid; gap:.75rem; } label { display:grid; gap:.3rem; } input, textarea { box-sizing:border-box; width:100%; padding:.55rem; border:1px solid var(--border); border-radius:calc(var(--radius) - .1rem); background:var(--surface-muted); color:var(--text); } textarea { min-height:10rem; resize:vertical; } .description { min-height:6rem; padding:.6rem; border:1px solid var(--border); border-radius:var(--radius); text-align:left; color:var(--text); background:var(--surface-muted); } .description span { color:var(--text-muted); } .three { display:grid; grid-template-columns:repeat(3, 1fr); gap:.5rem; } .check { display:flex; align-items:center; gap:.5rem; } .check input { width:auto; } aside { display:grid; align-content:start; gap:.75rem; min-width:0; } aside :global(section) { padding:.8rem; border:1px solid var(--border); border-radius:var(--radius); background:var(--surface-muted); } footer { display:flex; justify-content:end; gap:.5rem; margin-top:1rem; padding-top:1rem; border-top:1px solid var(--border); } footer.editor-actions { justify-content:space-between; } .markdown :global(p) { margin:.3rem 0; } @media (max-width:720px) { .main-grid, .three { grid-template-columns:1fr; } }
</style>
