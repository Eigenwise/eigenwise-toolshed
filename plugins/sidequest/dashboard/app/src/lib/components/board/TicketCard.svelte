...
<article class:has-story={Boolean(story)} class="card" draggable="true" ondragstart={() => { ondragging(true); state.setDragging(true); }} ondragend={() => { ondragging(false); state.setDragging(false); }}>
  ...
  {#if assets.length}<div class="thumbs">{#each assets.slice(0, 3) as filename (filename)}<button class="thumbnail" aria-label={`Open ${filename}`} onclick={() => showImage(filename)}><img loading="lazy" src={state.api.assetUrl(projectFor(ticket), ticket.id, filename)} alt="" onerror={(event) => event.currentTarget.classList.add('unavailable')} /><span class="thumbnail-label" aria-hidden="true">image</span></button>{/each}{#if assets.length > 3}<span>+{assets.length - 3}</span>{/if}</div>{/if}
</article>

<style>
  .card { position: relative; display: grid; gap: .45rem; overflow: hidden; padding: .75rem; border: 1px solid var(--border); border-radius: var(--radius); background: var(--surface); box-shadow: 0 2px 5px rgb(31 41 51 / .04); cursor: grab; transition: transform var(--motion-fast), box-shadow var(--motion-fast); } .card:hover { transform: translateY(-1px); box-shadow: var(--shadow); } .card:active { cursor: grabbing; } .card-main { display: grid; gap: .45rem; width: 100%; padding: 0; border: 0; background: transparent; color: inherit; text-align: left; font: inherit; cursor: pointer; } .story-rail { position: absolute; inset: 0 auto 0 0; width: 3px; } .topline, .footer { display: flex; align-items: center; flex-wrap: wrap; gap: .35rem; } code { color: var(--text-muted); font: .66rem var(--font-mono); } .project, .priority, .chip, .meta { padding: .12rem .32rem; border: 1px solid var(--border); border-radius: 999px; color: var(--text-muted); font-size: .64rem; line-height: 1.25; } .project { background: var(--surface-muted); } .priority { margin-left: auto; text-transform: uppercase; font-family: var(--font-mono); } .priority.urgent { color: #a52d2d; background: #f8e5e2; border-color: #eab9b4; } .priority.high { color: #8b5611; background: #fbf0dc; border-color: #ecd19d; } .story { color: var(--story); border: 1px solid color-mix(in srgb, var(--story), white 55%); border-radius: 999px; padding: .12rem .32rem; font-size: .64rem; } .category { color: var(--accent); font-size: .65rem; } strong { font-size: .9rem; line-height: 1.28; } .description { display: -webkit-box; overflow: hidden; color: var(--text-muted); font-size: .76rem; line-height: 1.4; line-clamp: 2; -webkit-box-orient: vertical; -webkit-line-clamp: 2; } .thumbs { display: flex; gap: .3rem; } .thumbs button, .thumbs > span { display: grid; width: 2.5rem; height: 2.5rem; place-items: center; overflow: hidden; border: 1px solid var(--border); border-radius:calc(var(--radius) - .25rem); background:linear-gradient(135deg, var(--surface-muted), var(--accent-soft)); color: var(--text-muted); cursor: pointer; font-size: .7rem; } .thumbnail { position:relative; isolation:isolate; } .thumbnail::before { content:''; position:absolute; inset:.22rem; border:1px solid color-mix(in srgb, var(--accent) 40%, transparent); border-radius:calc(var(--radius) - .4rem); } img { position:relative; z-index:1; width:100%; height:100%; object-fit:cover; transition:opacity var(--motion-fast); } img:global(.unavailable) { opacity:0; } .thumbnail-label { position:absolute; z-index:0; font:.56rem var(--font-mono); text-transform:uppercase; letter-spacing:.05em; } .footer { min-height: 1rem; } .chip.reply, .chip.reminder { color: #8b5611; background: #fbf0dc; border-color: #ecd19d; } .chip.blocked { color: var(--danger); background: #f8e5e2; border-color: #eab9b4; } .chip.claim { color: var(--accent); background: var(--accent-soft); } .chip.claim.stale { color: var(--text-muted); background: var(--surface-muted); } .chip.route { font-family: var(--font-mono); } .chip.label { background: var(--surface-muted); } .meta, time { color: var(--text-muted); font: .63rem var(--font-mono); } time { margin-left: auto; }
</style>
