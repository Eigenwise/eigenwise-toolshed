...
<style>
  ...
  .project-row { display: flex; align-items: center; gap: .4rem; min-width: 0; } .project-row b { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: .86rem; } .project-row small { margin-left: auto; color: var(--text-muted); font-family: var(--font-mono); font-size: .7rem; } mark { padding: .08rem .3rem; border-radius: 999px; background: var(--warning); color: var(--text-on-accent); font: 700 .66rem var(--font-mono); } .muted { color: var(--text-muted); font-size: .65rem; }
