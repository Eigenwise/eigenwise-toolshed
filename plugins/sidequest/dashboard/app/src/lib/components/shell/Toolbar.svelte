...
<header bind:this={toolbar} class="toolbar">
  <div class="title"><h1>{state.currentProject?.name ?? 'All boards'}</h1><p>{subtitle}</p></div>
  <div class="controls">
...
<style>
  .toolbar { display: flex; align-items: end; justify-content: space-between; gap: 1rem; margin-bottom: 1.5rem; } .title { flex:1 1 auto; min-width:0; } .title h1, .title p { margin: 0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; } .title h1 { font-family: var(--font-serif); font-size: clamp(1.35rem, 2vw, 1.85rem); letter-spacing: -.03em; } .title p { margin-top: .25rem; color: var(--text-muted); font-size: .8rem; }
  ...
  @media (max-width: 820px) { .toolbar { align-items: stretch; flex-direction: column; } .controls { justify-content: start; } .search { flex: 1 0 100%; order: -1; } .search input { width: 100%; } }
</style>
