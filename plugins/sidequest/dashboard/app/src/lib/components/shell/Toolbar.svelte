...
<style>
  ...
  .priority { display: flex; overflow: hidden; border: 1px solid var(--border); border-radius: var(--radius); } .priority button { border: 0; border-radius: 0; text-transform: capitalize; } .priority button + button { border-left: 1px solid var(--border); } .menu-wrap { position: relative; } .popover { position: absolute; z-index: 3; top: calc(100% + .35rem); right: 0; display: grid; min-width: 13rem; padding: .25rem; border: 1px solid var(--border-strong); border-radius: var(--radius); background: var(--bg-deep); } .popover button { display: flex; align-items: center; justify-content: space-between; gap: 1rem; border: 0; text-align: left; } .popover small { color: var(--text-muted); } .popover i { width: .55rem; height: .55rem; border-radius: 999px; } .new-ticket { border-color: var(--accent); background: var(--accent); color: var(--text-on-accent); font-weight: 700; } .new-ticket:hover { background: var(--accent-strong); color: var(--text-on-accent); }
