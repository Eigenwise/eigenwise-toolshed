...
export const CLAIM_REFUSAL_MESSAGES: Readonly<Record<string, RefusalMessage>> = Object.freeze({
  ...
  token: (ref) => `${ref} has a prepared dispatch whose token was missing or invalid. Do not retry dispatch from this executor or release a dispatch you did not claim. The orchestrator should run \`sidequest pulse ${ref}\`. If it reports launched with no bound runtime identity, claim, or checkpoint, run \`sidequest dispatch ${ref} --recovery-evidence "<observed failed-claim evidence>"\` once; otherwise wait for the active attempt to become terminal before dispatching again.`,
