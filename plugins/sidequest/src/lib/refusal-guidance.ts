...
export const CLAIM_REFUSAL_MESSAGES: Readonly<Record<string, RefusalMessage>> = Object.freeze({
  ...
  token: (ref) => `${ref} has a prepared dispatch whose token was missing or invalid, likely because it was transcribed incorrectly. Re-read the grouped lowercase token from this executor's briefing and retry the claim with its \`--token\`. Do not retry dispatch from this executor or release a dispatch you did not claim; only the orchestrator should re-dispatch after confirming the dispatch is terminal or stale.`,
