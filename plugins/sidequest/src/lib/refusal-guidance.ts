...
export const CLAIM_REFUSAL_MESSAGES: Readonly<Record<string, RefusalMessage>> = Object.freeze({
  ...
  unbound_dispatch: (ref) => `${ref} is an isolated dispatch without a bound harness agent identity, so its executor cannot claim it. Do not redispatch it from this executor or claim its token manually. Release ${ref} with a reason using \`sidequest release ${ref}\` so the orchestrator can redispatch it.`,
