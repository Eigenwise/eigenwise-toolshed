...
// noBounce says a live-claim remedy exists for this refusal, so the executor is not
// told to hand back when the orchestrator can widen the scope it is already holding.
function scopeRulingGuidance(res: any) {
...
function scopeRequestChanged(res: any) {
...
function scopeGrantChanged(res: any) {
  if (!res.ok) return null;
...
const tools: ToolDefinition[] = [
  ...
  },
  {
    ...
    handler(args) {
      ...
      if (args.grant) {
        if (args.files !== undefined) throw new Error('scopeRequest: grant cannot be combined with files — it grants the refusal the ticket already recorded.');
        const granted = store.grantScope(slug, args.ref, by, { source: 'mcp' });
        return mutationAck(slug, granted, scopeGrantChanged(granted));
      }
      if (args.files === undefined) throw new Error('scopeRequest: pass files, or grant:true to grant the outstanding refused request.');
