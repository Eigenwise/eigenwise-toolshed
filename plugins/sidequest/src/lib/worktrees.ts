...
async function lateContentInMovedWorktree(destination: string, classifiedHead: string | null, recordedLinks: readonly string[]): Promise<string | null> {
  const status = await git(destination, [...AT_RISK_STATUS_ARGUMENTS]);
  if (!status.ok) return `the moved tree could not be read again: ${status.stderr || `git status exited ${status.status}`}`;
  const held = atRiskStatusEntries(status.stdout, destination, recordedLinks);
  if (held.length) return `the moved tree holds ${held.length} entries the classification did not see, starting with ${held[0]!.code} ${held[0]!.path}`;
  const head = await git(destination, ['rev-parse', 'HEAD']);
  if (!head.ok) return `the moved tree's HEAD could not be read again: ${head.stderr || `git rev-parse exited ${head.status}`}`;
  if (classifiedHead && head.stdout !== classifiedHead) return `the moved tree is at ${shortCommit(head.stdout)}, not the classified ${shortCommit(classifiedHead)}`;
  return null;
}
...
async function recoveryStoreReport(store: RecoveryStoreName, root: string, protectedAgentIds: Set<string>, retentionAgeMs: number, options: any): Promise<RecoveryStoreReport> {
...
async function sweepRecoveryStores(tickets: any[], options: any): Promise<{ retentionAgeMs: number; quarantine: RecoveryStoreReport; failures: Array<{ path: string | null; message: string }> }> {
...
async function sweep(repo: string, tickets: any[], options: any = {}): Promise<any> {
  ...
  const retainedBranches: Array<{ branch: string; path: string }> = [];
  ...
  if (execute) {
    for (const entry of entries.filter((candidate) => candidate.action === 'remove' || candidate.action === 'salvage' || candidate.action === 'quarantine')) {
      ...
      const park = (reason: string, message: string) => {
      ...
      if (parking) {
        ...
        park(entry.reason, moveMessage);
        continue;
      }
      const classifiedReason = entry.reason;
      const lateContent = await lateContentInMovedWorktree(destination, entry.head, recordedLinks);
      if (lateContent) {
        park('late_content_quarantined', `classified ${classifiedReason}, but ${lateContent}, so the moved tree was parked instead of deleted`);
        continue;
      }
      const dependencyLinksReleased = releaseQuarantinedDependencyLinks(destination, recordedLinks);
      if (!dependencyLinksReleased.ok) {
        park(dependencyLinksReleased.reason, `classified ${classifiedReason}, but the moved tree still holds a dependency link, so it was parked instead of deleted`);
        continue;
      }
      try {
        ...
        park('quarantined_delete_failed', message);
        continue;
      }
      clearFailure(entry.path);
      removed.push(entry.path);
      // Unregister before the branch handling below: git refuses to delete a branch that is still
      // checked out by a registered worktree, even one whose directory has gone.
      const pruned = await git(repo, ['worktree', 'prune']);
      if (!pruned.ok) failures.push({ path: null, message: pruned.stderr || 'git worktree prune failed' });
      reportSweepProgress(options, entries, removed, sweepingStatus);
      const branch = localBranchName(entry.branch);
      if (!branch) continue;
