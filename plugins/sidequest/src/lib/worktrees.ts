...
async function lateContentInMovedWorktree(
  ...
  branch: string | null,
): Promise<{ blocked: string | null; branchTip: string | null }> {
  const blockedBy = (blocked: string) => ({ blocked, branchTip: null });
  ...
  if (!branch) return { blocked: null, branchTip: null };
  ...
  return { blocked: null, branchTip: tip.stdout };
}
  ...
  if (execute) {
    for (const entry of entries.filter((candidate) => candidate.action === 'remove' || candidate.action === 'salvage' || candidate.action === 'quarantine')) {
      ...
      if (movedRead.blocked) {
        park('late_content_quarantined', `classified ${classifiedReason}, but ${movedRead.blocked}, so the moved tree was parked instead of deleted`);
        continue;
      }
