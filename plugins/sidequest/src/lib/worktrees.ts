...
// `git worktree remove --force` follows a junction and deletes what it points at, so every link is
// unlinked before a removal.
  ...
  if (execute) {
    for (const entry of entries.filter((candidate) => candidate.action === 'remove' || candidate.action === 'salvage' || candidate.action === 'quarantine')) {
      ...
      if (entry.action === 'quarantine') {
        // Rename never follows a link, so the move itself is link-safe and the records are only
        // released once the tree has actually moved: a failed rename leaves the source byte-for-byte
        // AND record-for-record (SQ-2952).
