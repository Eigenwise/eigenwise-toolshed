...
function categoriesPayload(project?: any) {
  const usage = categoryUsageCounts(project);
  const global = store.getCategories();
  const globalById = new Map<any, any>(global.map((category?: any) => [category.id, category]));
  ...
  const effectiveById = new Map<any, any>((project && project !== 'all' ? store.getCategories({ project, withState: true }) : global).map((category?: any) => [category.id, category]));
  const ids = new Set([...globalById.keys(), ...localById.keys()]);
  const danglingOverrides = local.warnings.filter((warning?: any) => warning.kind === 'dangling-override');
  ...
  return {
    ...
    }).filter(Boolean).concat(danglingOverrides.map((warning?: any) => ({
      id: warning.id,
      name: warning.id,
  ...
  if (req.method === 'POST' && pathname === '/api/categories') {
    ...
    try {
      if (project) {
        ...
        const payload = categoriesPayload(project);
        sendJson(res, 201, { category: payload.categories.find((category?: any) => category.id === id), warnings: payload.warnings });
        return;
      }
