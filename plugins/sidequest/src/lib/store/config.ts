...
function integrationTarget(slug?: any, override?: any) {
  ...
  const target = { mode, upstream, branch,
    ...(requested.checkoutPath != null ? { checkout: captureIntegrationCheckout(meta.path, requested.checkoutPath, branch) }
      : Object.hasOwn(requested, 'checkout') ? { checkout: requested.checkout } : {}),
  };
...
// Reading the bare `origin/main` or `main` name lets a same-named tag shadow the
// intended ref, so the frozen target commit resolves through the qualified ref.
