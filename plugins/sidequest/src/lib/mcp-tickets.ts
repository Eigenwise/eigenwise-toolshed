...
function sameBasenameSiblingDetails(project: string, ticket: any, projectPath: string, tool: string) {
  ...
  return {
    sameBasenameSiblingDetails: {
      ...
      paths: details.reduce((total: number, detail: any) => total + detail.siblingPaths.length, 0),
...
const tools: ToolDefinition[] = [
  ...
  },
  {
    ...
    handler(args) {
      ...
      if (args.workingTreeDelivery !== undefined) patch.workingTreeDelivery = args.workingTreeDelivery;
      if (args.externalDeliverable !== undefined) patch.externalDeliverable = args.externalDeliverable;
