...
async function main(): Promise<void> {
  ...
  emit(
    '=== sidequest (active) ===\n' +
      ...
      checkpointingGuidance(data) +
      '\n' +
      'Reload the Sidequest skill before board work. SOLO-FIT FIRST: coherent, context-safe work with up to two real streams gets one ticket+executor. Otherwise use checkable ATOMIC tickets with anchors, contract, bounds, decisions, and verify; several deliverables are a smell, so plan then wave. Passing done-oracle skips audit/fix unless high-stakes; full suite once/wave. An external tracker like Jira still uses Sidequest.\n' +
      'Execution economy:\n' +
