...
function expectedDispatchDescription(input: HookInput): string | null {
  const toolInput = toolInputOf(input);
...
function main(): void {
  ...
  if (!hasModel) {
    ...
    if (result.status === 'ok' && result.model) {
      ...
      recordAuthoritativeLaunch(input, type, launchAgentName);
      writeJson({
        systemMessage: `sidequest: ${type} spawned without a model — injected "${result.model}" from ${result.refs.join(', ')}'s resolved category route. Always pass model: exec.model on Claude routes.`,
        hookSpecificOutput: { hookEventName: 'PreToolUse', updatedInput },
      });
      return;
    }
