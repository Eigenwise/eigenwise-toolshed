...
const WORKTREE_SWEEP_RECLAIM_RULE = 'nothing is deleted where it stands: a reclaimed tree is renamed into quarantine, read again there, and deleted only if it is still the clean tree that was classified, so anything written while the sweep ran stays parked. A tree counts as clean only when its status carries nothing untracked or ignored; installed files under an ignored node_modules, which worktree setup regenerates, are the one exception';
...
Native Agent dispatch (routed work stays in this conversation):
  ...
  sidequest worktrees <status|sweep> [--dry-run] [--yes] [--all-projects] [--min-age-hours N] [--recovery-retention-age-hours N] [--project <path-or-slug>]  report per-directory worktree and quarantine storage with a total; sweep classifies in this order: ${WORKTREE_SWEEP_CLASSIFICATION_ORDER.join(', ')}. ${WORKTREE_SWEEP_THRESHOLDS}. ${WORKTREE_SWEEP_RECLAIM_RULE}; anything else stays in quarantine. --yes removes planned stale worktrees and expired quarantine entries; --all-projects walks every registered project, oldest worktrees first
