...
const ARRAY_FLAGS = new Set(['image', 'label', 'file', 'add-file', 'remove-file', 'always-in-scope', 'read-only-denied-tool', 'auto-approve-scope', 'produces', 'changes', 'consumes', 'changed-surface', 'dependency']);
...
function sweepThresholdLabel(milliseconds: number): string {
  const hours = milliseconds / (60 * 60 * 1000);
  return hours >= 24 ? `${hours / 24} days` : `${hours} hours`;
}

// The reason order alone told an agent nothing about when each reason fires, so the numbers come
// from the same constants the sweep runs on (SQ-2952).
...
// Command help and the top-level help drifted apart on the node_modules exception, and an agent acts
// on whichever one it read (SQ-2958). Both render this sentence, and so do orchestration.md and the
...
const HELP_COMMANDS: any = {
  ...
  'scope-request': 'sidequest scope-request <id|SQ-n> --file path [--file path...] [--by who] | sidequest scope-request <id|SQ-n> --grant [--by who]  (orchestrator: grants the ticket\'s most recently refused request; same as sidequest scope-grant <id|SQ-n>)',
  'scope-grant': 'sidequest scope-grant <id|SQ-n> [--by who]  orchestrator-only: grants the ticket\'s most recently refused scope request, widening declared files without a redispatch',
...
Usage:
  ...
  sidequest update <id|SQ-n> [-t title] [-d desc|--body-file path] [-p priority] [-s status] [--file <path>|--file none]... [--add-file <path>]... [--remove-file <path>]... [--high-stakes[=false]] [-l label]... [--produces name]... [--changes name]... [--consumes name]... [--contract-waiver[=false]] [--readonly true|false] [-i image]... [--category <id|none>] [--route-model <model> --route-effort <effort>|--route none] [--complexity 1-10 --why "<motivation>"]
      --file replaces the whole declared list; --add-file/--remove-file adjust it in place without dropping the rest (refused together with --file).
...
Working the board safely (multi-agent):
  ...
  sidequest scope-request <id|SQ-n> --grant [--by who]  (or sidequest scope-grant <id|SQ-n>)   orchestrator-only: grants the ticket's most recently refused request outright, widening declared files without a redispatch
  ...
  sidequest update ... --add-file path [--add-file path...] / --remove-file path [--remove-file path...]   widen or
    trim the declared list without replacing it; refused together with --file. This is how the orchestrator answers
    a scope refusal without dropping the ticket's existing files — or grant the pending request outright with
    scope-grant/scope-request --grant instead.
...
async function main() {
  ...
  switch (cmd) {
    ...
    case 'scope_request':
      ...
      break;
    case 'scope-grant':
      await cmdScopeRequest(Object.assign({}, opts, { grant: true }), positional);
      break;
    case 'commit':
