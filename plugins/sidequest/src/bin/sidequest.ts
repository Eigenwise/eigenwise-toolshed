...
Native Agent dispatch (routed work stays in this conversation):
  ...
  sidequest worktrees <status|sweep> [--dry-run] [--yes] [--all-projects] [--min-age-hours N] [--recovery-retention-age-hours N] [--project <path-or-slug>]  report per-directory worktree and quarantine storage with a total; sweep classifies in this order: ${WORKTREE_SWEEP_CLASSIFICATION_ORDER.join(', ')}. ${WORKTREE_SWEEP_THRESHOLDS}. A tree is only removed when it holds nothing untracked or ignored (installed files under an ignored node_modules, which setup regenerates, do not count); anything else moves whole into quarantine. --yes removes planned stale worktrees and expired quarantine entries; --all-projects walks every registered project, oldest worktrees first
