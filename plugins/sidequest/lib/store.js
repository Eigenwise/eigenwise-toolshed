...
const { preferredWorktreeIntegrationTarget, agentWorktreePath, agentWorktreeCandidates, resolvedAgentWorktree } = require("./worktrees.js");
...
function spawnDescription(ticket, resolved) {
  const title = String(ticket && ticket.title || "Sidequest ticket").replace(/\[(?:switchboard-route|sidequest-route) model=[a-z0-9][a-z0-9.-]{0,63} effort=(?:low|medium|high|xhigh|max)\]/gi, " ").replace(/\s+/g, " ").trim() || "Sidequest ticket";
