...
function createRouting(dependencies) {
  ...
  function classifyDispatchFailure(error) {
    ...
    if (/\bstream idle timeout\b/.test(normalized)) return "stream_idle_timeout";
  ...
  function terminalAgentFailure(error) {
    ...
    return ["context_overflow", "max_tokens", "stream_idle_timeout", "worktree_environment", "agent_terminal"].includes(failureShape) ? failureShape : null;
  }
  ...
  function resolveTicketRoute(ticket, category) {
    const warnings = [];
