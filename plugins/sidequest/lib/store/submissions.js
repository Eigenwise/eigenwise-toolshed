...
${verify.outputTail}` : null
  ...
  function recordDeliveredSubmission(slug, idOrRef, opts) {
    ...
    try {
      ...
      const content = workingTreeDelivery ? workingTreeContainsSubmittedContent(repo, ticket.submission, deliveryCommit) : deliveryContainsSubmittedContent(repo, ticket.submission, deliveryCommit);
      if (!content.ok) {
        return {
          ok: false,
      ...
      const deliveredFiles = workingTreeDelivery ? workingTreeDeliveryPaths(repo) : deliveredCommitPaths(repo, deliveryCommit);
