...
function validProcessEntry(value) {
  if (!objectRecord(value)) return null;
  const pid = Number(value.pid);
  const parentPid = Number(value.parentPid);
  const command = typeof value.command === "string" ? value.command : "";
  return Number.isInteger(pid) && pid > 0 && Number.isInteger(parentPid) && parentPid > 0 ? { pid, parentPid, command } : null;
}
...
function sidequestMcpProcesses(options = {}) {
  const run = options.spawnSync || spawnSync;
  const processes = options.listProcesses ? options.listProcesses() : (options.platform || process.platform) === "win32" ? windowsProcesses(run) : posixProcesses(run);
  return processes.filter((entry) => /(?:^|[\\/])sidequest-mcp\.js(?:["\s]|$)/i.test(entry.command));
}
function stopProcess(pid, options = {}) {
...
function reapMcpSiblings(options = {}) {
  ...
  return sidequestMcpProcesses(options).filter((entry) => entry.pid !== processId && entry.parentPid === parentProcessId).flatMap((entry) => stopProcess(entry.pid, options) ? [entry.pid] : []);
}
