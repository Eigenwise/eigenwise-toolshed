...
function executorSafetyBody(ticket, nonce, tokenFile, project, executor, closeout, worktreeIdentity, readOnlyScratchSpace, worktreeSync) {
  ...
  return [
    ...
    verify,
    ticket.executorVerify && ticket.executorVerifyKind !== "attestation" ? "Run it through " + capturedVerifyCommand(ticket.executorVerify) + " in the FOREGROUND with an explicit generous timeout of up to 600000 ms. A backgrounded verify's completion does not wake you, so going idle on it parks the claim indefinitely. If it genuinely exceeds the 10-minute Bash ceiling, use bounded foreground until-loops instead of backgrounding or going idle; post [sidequest:verify-start] before it only for an expected no-op, and always post [sidequest:verify-complete] with status first after it exits." : "",
