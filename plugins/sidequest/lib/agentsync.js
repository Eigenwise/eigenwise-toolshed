...
function routeMarker(dispatchModel, effort, ticketRef) {
...
function ticketRouteMarker(ticket) {
  ...
  return resolved && resolved.backend === "codex" && resolved.dispatchModel ? routeMarker(resolved.dispatchModel, ticket.effort, ticket.ref) : null;
}
function ticketCloseout(ticket) {
  ...
  if (ticket?.dispatch?.workingTreeDelivery === true) {
    ...
    const verificationCloseout = requirement.command ? "Run the pinned verify-capture wrapper so it records the final working-tree candidate. done.verify cannot replace that capture." : `Pass explicit --verify evidence for the pinned ${requirement.kind || "custom"} requirement; blank or invalid evidence refuses and retains the claim.`;
    const doneVerify = requirement.command ? "" : ' --verify "<typed evidence>"';
    return `Closeout: this prepared shared-tree dispatch has a working-tree deliverable. Pre-existing dirty or staged paths were recorded at dispatch; leave them untouched and continue with the declared scope. Sidequest excludes a sibling only when its shared-tree dispatch records the same nonempty preparing session, its live claim overlaps yours, and its scope is disjoint. Before classifying dirty paths, it validates every eligible completed sibling's recorded candidate against its recorded paths and current content. A mismatch refuses closeout. Preserve the shared-tree work and hand it back to the existing parent for verification or grooming; never revert it, expand scope to absorb it, or commit it. Do not commit or submit. After the final declared-scope edit, ${verificationCloseout} Then close with done${doneVerify} --model ${resolved.runsModel} --effort ${effort}. Include changed paths and verification evidence in the completion comment. Then stop without a routine SendMessage.`;
  }
  ...
  const externalDeliverableVerify = externalDeliverableRequirement.command ? "the pinned verify-capture wrapper records the current dispatch attempt and revision" : `explicit done --verify evidence for the pinned ${externalDeliverableRequirement.kind || "custom"} requirement is supplied`;
...
function dispatchBoundWorktree(ticket) {
  const dispatch = ticket?.dispatch;
  if (!dispatch || dispatch.sharedTree !== false) return null;
  const worktree = String(dispatch.worktree || "").trim();
  return worktree ? canonicalPath(worktree) : null;
}
