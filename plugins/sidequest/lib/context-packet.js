...
const MAX_MCP_OUTPUT_TOKENS = 25e3;
...
function contextRevision(value) {
  return `ctxr1.${sha256(value)}`;
}
function encodedContextValue(prefix, value) {
  return `${prefix}${Buffer.from(canonicalJson(value), "utf8").toString("base64url")}`;
}
function decodedContextValue(value, prefix, label) {
...
function createContextHandle(source) {
...
function decodeContextHandle(handle) {
...
function contextCursor(handle, position) {
...
function decodeContextCursor(handle, cursor) {
...
function contextRetrieval(source, cursorPosition = 0) {
...
function contextPageByteLimit(value) {
...
function utf8Slice(value, startBytes, maxBytes) {
  const source = String(value ?? "");
...
function rowsWithinByteLimit(rows, start, maxBytes) {
...
function includedItem(item, body) {
  return {
    id: item.id,
    kind: item.kind,
    body,
