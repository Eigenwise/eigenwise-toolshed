  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        recoveryEvidence: { type: "string", description: "Observed failure evidence. With claimHolder, executor, and worktree, recover that live isolated claim without releasing it. With retireOnly:true, retire an eligible unclaimed prepared or launched attempt before runtime binding, or one bound and unclaimed past the claim grace with no claim, checkpoint, or claim activity, without preparing a replacement. The claim is a bound executor’s first call, so past that grace the evidence the host reported the runtime gone is enough; inside it the refusal names the exact instant it becomes retirable." },
    ...
    handler(args) {
      ...
      return {
        ...
        spawnDescriptionNote: dispatchState.reducedAgentSchema === true ? `Copy spawn.description byte-for-byte into the Agent call. This reduced-schema spawn intentionally omits name and mode; the board retains launch label "${dispatchState.launchName}" separately, and its first claim requires hook-reported agent_id plus a permission_mode of "auto" or "bypassPermissions" (the executor inherits this session's mode, so do not change it).` : `Copy spawn.description byte-for-byte into the Agent call. It leads with "${description.split(" · ")[0]}" for notifications, while the stable spawn.name ends with the resolved route token and effort.`
      };
    }
  },
  {
