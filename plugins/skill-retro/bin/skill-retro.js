...
const USAGE = `skill-retro - mine recent Claude Code transcripts for work that keeps getting redone

Usage:
  skill-retro mine [options]
  skill-retro salvage --dir <report-dir> --id <salvage-id> --to <file>
  skill-retro verify  --dir <report-dir> [--id <salvage-id>] [--run]
...
function defaultOutputDir(options) {
  ...
  return path.join(os.tmpdir(), 'skill-retro', `${slug}-${stamp}`);
}

function loadReport(dir) {
  const file = path.join(dir, 'findings.json');
  if (!fs.existsSync(file)) throw new Error(`No findings.json in ${dir}. Run \`skill-retro mine --out ${dir}\` first.`);
...
if (require.main === module) {
  main().catch((error) => {
    process.stderr.write(`skill-retro failed: ${error.message}\n`);
    process.exitCode = 1;
  });
}
