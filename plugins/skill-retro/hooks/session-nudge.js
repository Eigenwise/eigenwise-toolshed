...
function enabled(env) {
  return TRUTHY.has(String(env.SKILL_RETRO_NUDGE ?? '').trim().toLowerCase());
}

function every(env) {
  const value = Number(env.SKILL_RETRO_NUDGE_EVERY);
...
function stateFile(root, env) {
  ...
  return path.join(home, 'skill-retro-state', `${slug}.json`);
}
...
function main() {
  ...
  process.stdout.write(JSON.stringify({
    hookSpecificOutput: {
      ...
      additionalContext:
        `[skill-retro] ${since} sessions have ended since the last transcript retro.${lastRun}`
        + ' Suggest running the skill-retro skill when the user has a moment; do not start one unprompted.',
    },
  }));
}
