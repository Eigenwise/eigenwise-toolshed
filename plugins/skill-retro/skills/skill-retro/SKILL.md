...
- **Scripts parse.** `skill-retro verify --dir <report-dir>` runs a syntax check on every salvaged file.
...
## How this differs from `workbench:retro`

`workbench:retro` reflects on the session in your context right now: subjective, immediate, no disk
...
session's context can see, that is `workbench:retro`; if the question is "what do we keep redoing," it is
this.
