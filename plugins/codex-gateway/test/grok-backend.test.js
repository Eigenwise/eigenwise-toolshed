...
test('refreshes fixture auth without exposing it in return values', async (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-gateway-grok-'));
...
test('reads the CLI version and sends the required Grok headers', (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-gateway-grok-version-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
...
test('the shim lists and routes claude-prefixed Grok models', async (t) => {
  ...
  const authDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-gateway-grok-auth-'));
