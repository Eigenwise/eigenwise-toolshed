...
function request(port, pathname, body, sessionId) {
  return new Promise((resolve, reject) => {
    const headers = body
...
test('dispatch model reuses a session route after compaction and logs cache hits', async (t) => {
  ...
  assert.deepEqual(routes.map(({ model, via, effort, sessionId }) => ({ model, via, effort, sessionId })), [
    { model: 'gpt-5.6-sol', via: 'dispatch', effort: 'xhigh', sessionId: 'canonical-session' },
    { model: 'gpt-5.6-sol', via: 'dispatch-cached', effort: 'xhigh', sessionId: 'canonical-session' },
    { model: 'gpt-5.6-terra', via: 'dispatch', effort: 'high', sessionId: 'legacy-session' },
    { model: 'gpt-5.6-terra', via: 'dispatch-cached', effort: 'high', sessionId: 'legacy-session' },
  ]);
});
