...
async function processInfoAsync(pid, { commandResult = commandResultAsync, probeChildren = null } = {}) {
  ...
  const result = await commandResult(WIN ? 'powershell.exe' : 'ps', WIN
    ...
    : ['-ww', '-p', String(pid), '-o', 'pid=,ppid=,lstart=,args='], { probeChildren });
  if (result.timedOut) return undefined;
  return result.status === 0 ? processInfoFromOutput(result.stdout) : null;
}
...
module.exports = {
  ...
  processInfoAsync, processInfoSync, processIsOwnedByThisInstall, processIsOwnedByThisInstallAsync, processOwningPort: processOwningPortSync, processOwningPortAsync, processTableAsync, processTableSync,
