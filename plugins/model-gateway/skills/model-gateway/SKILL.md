...
- Typed: `/model claude-gpt-5.6-sol[1m]`. The picker and Sidequest catalog emit that exact id, so use it when selecting a Codex model manually too. The suffix is stripped before the request reaches Codex.
