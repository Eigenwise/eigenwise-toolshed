...
| `prune-observability.js` | Previews observations older than 90 days by default. Use `--retention-days <days>` to change the window, `--dry-run` as an explicit preview, then `--apply` (or `--yes`) to permanently delete the matching rows. `--help` lists every option and marks destructive flags. SQLite keeps the file size until a manual `VACUUM`, which needs roughly the database's current size in free disk space. |
