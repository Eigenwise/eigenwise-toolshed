...
After consent, every startup/resume launches a fail-open background ensure pass. It restores the observer and Collector when their configured ports are quiet, adopts or heals the configured dashboard container when Docker is present, and refreshes managed runtime files after a plugin update. The observer drains its spool and downstream outbox continuously. Users do not start these processes manually.
