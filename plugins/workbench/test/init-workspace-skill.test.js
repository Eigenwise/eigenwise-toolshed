...
test('init-workspace no longer configures Sidequest routing', () => {
  // Sidequest 4.0.0 removed routing profiles, categories, and worktree isolation.
  // Setup must not ask about, create, or apply any of them.
  for (const dead of [/routing profile/i, /sidequest profile/, /category routing/i, /worktreeIsolation/, /creative-music/]) {
    assert.doesNotMatch(skill, dead, `init-workspace still references ${dead}`);
  }
  assert.match(skill, /bring up the board \(`sidequest dashboard`, or ask the board skill\)/);
});
