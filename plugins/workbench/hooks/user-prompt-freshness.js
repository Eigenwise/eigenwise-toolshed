...
function blockReason(instances, state) {
  ...
  return `Toolshed update required before Claude works on this prompt. Outdated here: ${shown.join('; ')}${extra}. Run /update-toolshed, then /reload-plugins --force or restart Claude Code. This prompt was not sent to Claude. Update, reload, and /plugin maintenance paths remain allowed.`;
}
...
function reloadReason(instances, loadedVersion, reloadState, sessionId) {
  ...
  return installed ? `Toolshed plugins were updated, but this session still loaded workbench ${loadedVersion} while the installed version is ${installed.version}. Run /reload-plugins --force or restart Claude Code, then resubmit this prompt. This prompt was not sent to Claude.` : null;
}
