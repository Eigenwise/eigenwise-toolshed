...
function clientForRoot(rawRoot) {
  const canonical = canonicalizeRoot(rawRoot);
  if (canonical.error) {
    sweep();
    return canonical;
  }
  const existing = clients.get(canonical.key);
  if (existing && existing.client.isAlive()) {
    existing.client.touch();
    return { client: existing.client, rootDir: canonical.rootDir };
  }
  if (existing) clients.delete(canonical.key);
  const recipe = locateLanguageServer(canonical.rootDir);
  if (recipe.error) return { error: recipe.error };
  const client = createLspClient({
    rootDir: canonical.rootDir,
    recipe,
    onExit: () => {
      const entry = clients.get(canonical.key);
      if (entry && entry.client === client) clients.delete(canonical.key);
    },
  });
  clients.set(canonical.key, { client, rootDir: canonical.rootDir });
  ensureSweepTimer();
