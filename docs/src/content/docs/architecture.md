...
flowchart LR
  ...
  C --> G[Local dashboard]
  CC --> S[Sidequest board]
  CC --> W[Model Gateway shim]
  W --> P[Local proxy]
  P --> API[Claude or Codex backend]
```

See [modular toolshed architecture](./architecture/modular-architecture/) for the file-based integration points between plugins.
