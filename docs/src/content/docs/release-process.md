...
The queue shows held fragments, the latest release, integration and published branches, and the next scheduled-cut hint. A publish lock protects the cut and the push:

```text
sidequest publish lock --by <who>
# run the local release cut workflow
sidequest publish status
