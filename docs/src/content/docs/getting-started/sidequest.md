...
`sidequest claims sweep` (also run at session start, and available as the `sweepClaims` MCP tool) releases a claim in three cases: its executor has a durable `died` outcome, it went quiet past `SIDEQUEST_CLAIM_IDLE_MIN` (default 60 minutes) with no executor associated, or nothing ever reported the stop and it stayed quiet past `SIDEQUEST_CLAIM_ABANDON_MIN` (default 1440 minutes). Quiet time counts from the holder's last board write, so it says nothing about whether the process is alive. `sidequest pulse SQ-3` reports that value as `claim.boardQuietMs`, puts `reclaimable` and a recorded death first, and derives `liveness` from explicit evidence. An active verification marker is `alive`, pending scope is `waiting`, a death record is `dead`, and a bound claim without a process heartbeat is `unknown`. Leave an `unknown` claim alone.
