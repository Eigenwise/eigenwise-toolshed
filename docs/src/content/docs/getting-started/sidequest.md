...
For deliberate orchestrator-owned inline work, claim the ticket directly with a concrete recorded reason: `sidequest claim SQ-3 --by <unique-worker-id> --direct --reason "Integration gate pinpoints this exact mechanical diff"` (MCP: `direct:true` with `reason`). The reason must be at least 20 characters and is limited to the inline-safe allowlist the skill documents (pinpointed integration mechanical fixes, release bookkeeping, user-directed edits to one or two named files); rationales like "context already loaded" or "small change" are refused at claim time. Do not start either path until its claim succeeds.
