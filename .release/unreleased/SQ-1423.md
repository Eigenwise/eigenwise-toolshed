...
`--dry-run` reports what it would remove and how much space that reclaims, and
says up front that SQLite does not hand space back to the filesystem without
`VACUUM` — which needs free space roughly equal to the database.
