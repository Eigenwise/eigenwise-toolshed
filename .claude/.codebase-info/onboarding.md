...
2. Identify the surface: Sidequest engine/dashboard, Workbench, Observability, Model Gateway, Live-rules, Codebase Mapper, shared test support, docs, the local-only Windows Sandbox harness, or release automation. Start with [entry-points](entry-points.md) and [modules](modules.md).
...
4. For Sidequest lifecycle work, start with `plugins/sidequest/src/lib/kernel/`, then follow the adapter into `store/dispatch.ts`, `store/submissions.ts`, or `worktrees.ts`. `prepared-dispatch.ts` canonicalizes prepared executor identity, and `source-revision-capability.ts` binds immutable non-Git candidates to their dispatch baseline. Frozen briefing snapshots are continued with the `context_page` MCP tool.
