...
- Sidequest hooks: `plugins/sidequest/hooks/hooks.json`; source hooks are under `plugins/sidequest/src/hooks/`. Five events: PreToolUse safety guards (`guard-task-output`, `guard-peer-message`, `guard-home-delete`, `guard-bash-windows-paths`, `guard-heredoc-isolated`, `guard-powershell-cmd-shims`, `guard-destructive-git`, `guard-shared-tree-commit`, `guard-worktree-isolation`), Stop compaction suggestion, PreCompact `pin` policy, PostCompact recovery, and SessionStart registry write plus a 4 KB bounded board briefing. That briefing treats an enabled board as dispatch authorization, keeps specific one-file or one-prompt asks inline, and tells orchestrators to shard independent work concurrently.
