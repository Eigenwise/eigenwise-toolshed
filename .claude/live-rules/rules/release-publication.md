...
- Acquire the `sidequest publish lock`, run `node scripts/release/cut.mjs` with a clean tree, and run `--dry-run` first. Push with the exact `git push --atomic` command it prints, then unlock.
